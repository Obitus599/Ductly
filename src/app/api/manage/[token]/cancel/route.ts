...
export async function POST(
  ...
  const { data: booking, error: fetchError } = await supabase
    .from("bookings")
    .select("id, status, slot_start, payment_intent_id, team_id")
    .eq("manage_token", token)
    .returns<{ id: string; status: string; slot_start: string; payment_intent_id: string | null; team_id: string | null }[]>()
    .single();
